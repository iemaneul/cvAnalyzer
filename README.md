...
The account page lets authenticated users update their name and email or securely replace their password after confirming the current one.
...
- `PATCH /api/auth/profile` — update the authenticated user's name and email
- `PATCH /api/auth/password` — change password after verifying the current password
