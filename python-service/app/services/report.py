...
def build_report(analysis: dict) -> bytes:
    ...
    story = [
        Paragraph("Resume Analyzer Report", styles["ReportTitle"]),
        Paragraph(str(analysis.get("fileName", "Resume")), styles["Heading3"]),
        Spacer(1, 6),
    ]
    ...
    skill_rows = [["Matched skills", ", ".join(_strings(analysis.get("matchedSkills"))) or "None"], ["Missing skills", ", ".join(_strings(analysis.get("missingSkills"))) or "None"]]
    ...
    if action_plan:
        for index, item in enumerate(action_plan, 1):
            ...
            story.append(Paragraph(f"<b>{index}. [{priority}] {item.get('title', '')}</b>", styles["BodyText"]))
            story.append(Paragraph(str(item.get("description", "")), styles["SmallMuted"]))
    ...
    if suggestions:
        ...
        story.extend(Paragraph(f"- {item}", styles["BodyText"]) for item in suggestions)
    story.extend([
        Spacer(1, 12),
