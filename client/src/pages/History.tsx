...
export function History() {
  ...
  const filters = { search: deferredSearch || undefined, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined, ...scoreFilters };
  const { data, isLoading, error } = useAnalyses(page, 10, filters);
  const hasFilters = Boolean(search || dateFrom || dateTo || scoreRange !== 'all');
  const resetFilters = () => { setSearch(''); setScoreRange('all'); setDateFrom(''); setDateTo(''); setPage(1); };
  return <div>
