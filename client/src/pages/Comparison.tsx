...
export function Comparison() {
  ...
  return <div>
    ...
    <div className="mt-6 flex flex-col items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 sm:flex-row sm:justify-between">
      <div className="text-center sm:text-left"><small className="text-slate-400">Previous</small><p className="font-semibold">{data.previous.jobTitle ?? data.previous.fileName}</p>{data.previous.company && <p className="text-sm text-slate-500">{data.previous.company}</p>}<strong>{data.previous.score}%</strong></div>
      <ArrowRight className="rotate-90 text-indigo-500 sm:rotate-0" />
      <div className="text-center sm:text-right"><small className="text-slate-400">Current</small><p className="font-semibold">{data.current.jobTitle ?? data.current.fileName}</p>{data.current.company && <p className="text-sm text-slate-500">{data.current.company}</p>}<strong>{data.current.score}%</strong></div>
    </div>
